...
export default function AdminStudentsPage() {
  const [rows, setRows] = useState<
    { id: string; email: string; full_name: string; student_id: string; major: string | null; gpa: number | null }[]
  >([])
                    ...
                    <tbody>
                      {rows.map((r) => (
                        <tr key={r.id} className="border-b border-gray-100">
                          ...
                          <td className="py-2 pr-4">{r.gpa ?? "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <CardContent>
              <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="id" name="id" required placeholder="22511999" />
                </div>
                <div>
                  <Label htmlFor="student_id">University student #</Label>
                  <Input id="student_id" name="student_id" required placeholder="STU099" />
                </div>

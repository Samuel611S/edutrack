...
export async function PATCH(request: NextRequest, context: Params) {
  ...
  const db = getDb()
  const req = db
    .prepare("SELECT id, student_id, course_id, request_type, status FROM enrollment_requests WHERE id = ?")
    .get(requestId) as
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(
      `UPDATE enrollment_requests
       SET status = ?, reviewed_by = NULL, reviewed_by_role = 'admin', reviewed_by_name = ?, reviewed_at = CURRENT_TIMESTAMP
       WHERE id = ?`,
    ).run(action === "approve" ? "approved" : "rejected", session.name, requestId)
  })
  tx()
